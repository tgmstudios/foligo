...
router.post('/', [
  ...
  try {
    ...
    if (subdomain) {
      const available = await checkSubdomainAvailable(prisma, subdomain);
      if (!available) {
    ...
    const project = await prisma.project.create({
      ...
      include: {
        ...projectInclude({ content: { orderBy: { order: 'asc' } } }),
        siteConfig: true
      }
    });
...
router.get('/:id', authorizeProjectAccess('VIEWER'), async (req, res) => {
  try {
    ...
    const fullInclude = {
      ...projectInclude(),
      siteConfig: true,
      _count: { select: { content: true, members: true, media: true } }
    };
...
router.put('/:id', [
  ...
  try {
    ...
    const project = await prisma.project.update({
      where: { id },
      data: updateData,
      include: projectInclude({ content: { orderBy: { order: 'asc' } } })
    });
