...
function createCoverLetterEditorTools(doc, fetchPost, fetchResumeDocument) {
  ...
  return {
    ...
    write_cover_letter: tool({
      description: 'WHOLE-DOCUMENT REPLACE for the cover letter. Overwrites the entire LaTeX source at once — anything not included in `latex` is discarded. Use ONLY for: (a) the first draft, when no document exists yet, or (b) a large restructure touching most of the document. For any small change (wording, one paragraph, a detail) use edit_cover_letter_section instead — it is safer and shows exactly what changed. `latex` must always be a complete, valid, compilable .tex document (including \\documentclass and \\begin{document}...\\end{document}), never a fragment.',
      inputSchema: z.object({
        latex: z.string().describe('The complete new LaTeX source for the cover letter, from \\documentclass through \\end{document}. Not a partial snippet.'),
      }),
      execute: async ({ latex }) => {
        doc.content = latex;
        return 'Document replaced. The entire cover letter now contains exactly the LaTeX source you just wrote.';
      },
    }),

    edit_cover_letter_section: tool({
      description: 'TARGETED EDIT for the cover letter: finds one exact, unique occurrence of existing LaTeX text and replaces it, leaving the rest of the document untouched. This is the PREFERRED tool for small changes (wording, a paragraph, a detail) — use write_cover_letter only for a first draft or a large restructure. Before calling this, you must know the CURRENT exact text in the document (from earlier in this conversation or a prior tool result) — `search` is matched verbatim, including whitespace, against the document as it exists right now, not against what you assume it says. It fails loudly (with a reason) if the text isn\'t found or isn\'t unique, so you can retry with more context — it never silently does nothing.',
      inputSchema: z.object({
