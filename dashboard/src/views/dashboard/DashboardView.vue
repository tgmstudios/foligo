      ...
      <div class="space-y-6">
        ...
        <div class="card">
          ...
          <div class="p-6 space-y-3">
            ...
            <button
              @click="handleCreatePostLinks"
              :disabled="!selectedProject || isGeneratingLinks"
              class="w-full btn btn-outline group relative disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <svg v-if="!isGeneratingLinks" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
              </svg>
              <svg v-else class="h-4 w-4 mr-2 animate-spin" fill="none" viewBox="0 0 24 24">
...
const handleCreatePostLinks = async () => {
  // Get the first project or selected project
  const project = selectedProject.value || projectStore.projects[0]
  ...
  try {
    ...
    if (response.data.success) {
      ...
      if (created > 0) {
        toast.success(`Successfully created ${created} post link${created !== 1 ? 's' : ''}!`)
...
onMounted(async () => {
  ...
  if (projectStore.projects.length > 0 && !selectedProject.value) {
    selectedProject.value = projectStore.projects[0]
  }
})
</script>
