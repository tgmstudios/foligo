<template>
  <div class="min-h-screen bg-gray-900">
    <!-- Sidebar -->
    <div
      class="fixed inset-y-0 left-0 z-50 w-64 bg-gray-800 shadow-lg transform transition-transform duration-300 ease-in-out"
      :class="
        sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
      "
    >
      <div
        class="flex items-center justify-between h-16 px-6 border-b border-gray-700"
      >
        <router-link
          to="/"
      ...
      <nav class="mt-6 px-3">
        ...
        <div v-if="isSettingsRoute" class="space-y-1">
          ...
          <div
            v-if="authStore.user?.isAdmin"
            class="my-4 border-t border-gray-700"
          ></div>
        ...
        <div v-else class="space-y-1">
          <template v-for="item in navigation" :key="item.name">
            ...
            >
              ...
              <!-- Blogs Icon -->
              <svg
                v-else-if="item.icon === 'ContentIcon' && item.name === 'Blogs'"
                class="mr-3 h-5 w-5"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              ...
              <svg
                v-else-if="
                  item.icon === 'ContentIcon' && item.name === 'Experience'
                "
                class="mr-3 h-5 w-5"
                fill="none"
                viewBox="0 0 24 24"
              ...
              <svg
                v-else-if="
                  item.icon === 'ProjectsIcon' && item.name === 'Projects'
                "
                class="mr-3 h-5 w-5"
                fill="none"
                viewBox="0 0 24 24"
              ...
              <svg
                v-else-if="
                  item.icon === 'ProjectsIcon' && item.name === 'Portfolios'
                "
                class="mr-3 h-5 w-5"
                fill="none"
                viewBox="0 0 24 24"
              ...
              <!-- Analytics Icon -->
              <svg
                v-else-if="item.icon === 'AnalyticsIcon'"
                class="mr-3 h-5 w-5"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              ...
              <!-- Resume Icon -->
              <svg
                v-else-if="item.icon === 'ResumeIcon'"
                class="mr-3 h-5 w-5"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              ...
              <!-- GoApply Icon -->
              <svg
                v-else-if="item.icon === 'GoApplyIcon'"
                class="mr-3 h-5 w-5"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
        ...
        <div v-if="!isSettingsRoute" class="mt-8 pt-6 border-t border-gray-700">
          ...
          <div class="space-y-1">
            ...
            <router-link
              ...
              class="sidebar-item"
              :class="
                String($route.name).startsWith('goapply')
                  ? 'sidebar-item-active'
                  : 'sidebar-item-inactive'
              "
            >
      ...
      >
        ...
        <div
          v-if="showUserMenu"
          class="absolute bottom-16 left-4 right-4 bg-gray-700 rounded-md shadow-lg border border-gray-600 py-1 z-50"
        >
          <router-link
            v-if="authStore.user?.isAdmin"
            to="/admin"
    ...
    <div class="lg:pl-64">
      <!-- Top bar -->
      <div
        class="sticky top-0 z-40 bg-gray-800 shadow-sm border-b border-gray-700"
      >
        <div
        ...
        >
          <div class="flex items-center">
            ...
            <h2 class="ml-2 text-lg font-semibold text-white">
              {{ pageTitle }}
            </h2>
          </div>
          ...
          <div class="flex items-center space-x-4">
            ...
            <button
              ...
              class="hidden md:flex items-center w-64 pl-10 pr-3 py-2 border border-gray-600 rounded-md bg-gray-700 text-gray-400 hover:text-gray-300 hover:border-gray-500 transition-colors relative text-sm"
            >
              <span
              ...
              <kbd
                class="text-[10px] border border-gray-600 rounded px-1.5 py-0.5"
                >⌘K</kbd
              >
            </button>
          </div>
...
const pageTitle = computed(() => {
  const routeNames: Record<string, string> = {
    ...
    "goapply-resume": "GoApply · Resumes",
...
const handleContentGenerated = async (data: {
  content: string;
  title?: string;
  excerpt?: string;
  metadata: any;
  ...
  try {
    ...
    const excerpt =
      data.excerpt ||
      data.content.substring(0, 200).replace(/\n/g, " ").trim() + "...";
    ...
    if (contentType === "PROJECT") {
      if (data.metadata.startDate) {
        ...
        contentData.startDate = isNaN(startDate.getTime())
          ? null
          : startDate.toISOString();
      }
      if (data.metadata.endDate) {
        const endDate = new Date(data.metadata.endDate);
        contentData.endDate = isNaN(endDate.getTime())
          ? null
          : endDate.toISOString();
      }
      if (data.metadata.isOngoing !== undefined)
        contentData.isOngoing = data.metadata.isOngoing;
      if (data.metadata.projectLinks)
        contentData.projectLinks = data.metadata.projectLinks;
      if (data.metadata.contributors)
        contentData.contributors = data.metadata.contributors;
      if (data.metadata.featuredImage)
        contentData.featuredImage = data.metadata.featuredImage;
    }

    if (contentType === "EXPERIENCE") {
      if (data.metadata.experienceCategory)
        contentData.experienceCategory = data.metadata.experienceCategory;
      if (data.metadata.startDate) {
        const startDate = new Date(data.metadata.startDate);
        contentData.startDate = isNaN(startDate.getTime())
          ? null
      ...
      if (data.metadata.locationType)
        contentData.locationType = data.metadata.locationType;
    }

    if (contentType === "SKILL") {
      if (data.metadata.categoryTag)
        contentData.metadata = { categoryTag: data.metadata.categoryTag };
    }

    const newContent = await projectStore.createContent(
    ...
    if (data.skills && Array.isArray(data.skills) && data.skills.length > 0) {
      try {
        ...
        await api.post(
          `/projects/${selectedProjectId.value}/content/${newContent.id}/skills`,
          {
            skillIds: skillIds,
          },
        );
    ...
    if (data.tags && Array.isArray(data.tags) && data.tags.length > 0) {
      try {
        ...
        await api.post(
          `/projects/${selectedProjectId.value}/content/${newContent.id}/tags`,
          {
            tagIds: tagIds,
          },
        );
    ...
    if (
      ...
      data.metadata.roles &&
      Array.isArray(data.metadata.roles) &&
      data.metadata.roles.length > 0
    ) {
      try {
        ...
        for (const roleData of data.metadata.roles) {
          ...
          await api.post(
            `/projects/${selectedProjectId.value}/content/${newContent.id}/roles`,
            {
    ...
    // Navigate to the content editor
    router.push(
      `/portfolios/${selectedProjectId.value}/content/${newContent.id}/edit`,
    );
  } catch (error) {
